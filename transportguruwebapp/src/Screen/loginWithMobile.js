...
import { sendSms } from "../Redux/Action/loginAction";
const theme = createTheme();
const LoginWithMobile = (props) => {
    const nav = useNavigate()
    const [mobileNo, setMobileNo] = React.useState('')
    const sendSmsData = (val) => {
        setMobileNo(val)
        props.sendSms(val)
    }
    ...
    React.useEffect(() => {
        if (props.otpData.status) {
            props.reset()
            nav('/Otp', { state: { type: 'mobileno', item: mobileNo } }, { replace: true })
        }
    }, [props, mobileNo, nav])
    return (
        <ThemeProvider theme={theme} >
            <Grid container component="main" sx={{ height: "100vh" }}>
                ...
                <Grid item xs={12} sm={7} md={6}>
                    ...
                    >
                        <CssBaseline />
                        <SignInWithPhone onSendSms={(val) => { sendSmsData(val) }} />
                        <Divider  > Or</Divider>
                        <Box
                            sx={{
                                marginTop: 2,
...
export default connect(useSelector, useDispatch)(LoginWithMobile);
