...
const EditAccount = props => {
    ...
    const VerifyUser = () => {
        if (data.username === '') {
            ...
            if (
                ...
                props.usereditAccount({
                    data: { username, address: UserAddress, image: firebaseImage },
                    id: props.route.params.item?._id,
                    token: props.token,
                });
            }
    ...
    const Finish = () => {
        if (otp.length === 4) {
            if (!props.route.params.item.emailVerify) {
                props.userEmailVerify({
                    data: { email: data.email, otp: otp },
                    token: props.token,
                });
            }
            if (!props.route.params.item.mobileNoVerify) {
                props.userMobileNoVerify({
                    data: { mobileno: `${value}${data.mobileno}`, otp: otp },
                    token: props.token,
                });
            }
        } else {
