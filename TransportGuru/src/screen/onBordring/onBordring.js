...
function Onboardring({ navigation }) {
    ...
    const reanderitem = ({ item }) => {
        ...
        return (
            <View style={{ flex: 1 }}>
                <StatusBar hidden />
                <View style={{ backgroundColor: '', height: '70%', width: Dimensions.get('window').width }}>
                    <LottieView source={item.image} autoPlay loop />
                </View>
                <View style={{ backgroundColor: 'white', height: '30%' }}>
                    <Text style={{ textAlign: 'center', fontSize: 20, fontWeight: 'bold', marginVertical: 25 }}>{item.title}</Text>
