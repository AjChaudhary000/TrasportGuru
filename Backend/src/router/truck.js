...
router.post("/truck/create", auth, async (req, res) => {
    try {
        const truckdetails = new TruckDetails({
...
router.get("/truck", auth, async (req, res) => {
    try {
        const data = await TruckDetails.find({
            tarsportUserId: req.user._id,
            deleteData: false,
        })
...
router.delete("/truck/delete/:_id", auth, async (req, res) => {
    try {
        const truck = await TruckDetails.findByIdAndUpdate(
            { _id: req.params._id },
            { deleteData: true },
            { new: true }
        );
...
router.patch("/updatetruck/:_id", auth, async (req, res) => {
    try {
        const data = await TruckDetails.findByIdAndUpdate(
            { _id: req.params._id },
            req.body,
            { new: true }
        );
