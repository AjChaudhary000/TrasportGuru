...
router.patch("/route/update/:_id", auth, async (req, res) => {
    try {
        const data = await Route.findByIdAndUpdate(
            { _id: req.params._id },
            req.body,
            { new: true }
        );
...
router.post("/searchRoute", auth, async (req, res) => {
    try {
        ...
        const routeList = data
            .filter(
                (item) =>
                    item.truckId.truckCapicity - item.capicity >= req.body.capicity
            )
            .map((item) => {
                if (
                    item.routeId.from.name === req.body.from &&
                    item.routeId.destination.name === req.body.destination
                ) {
                    ...
                    if (item.routeId.from.name === req.body.from) {
                        const destination = item.routeId.routeStop.filter(
                            (destination) =>
                                destination.stops === req.body.destination &&
                                item.Truckdate.setHours(
                                    item.Truckdate.getHours() + destination.avgTime
                                ) > new Date()
                        );
                        if (destination.length !== 0) return item;
                    } else if (item.routeId.destination.name === req.body.destination) {
                        const from = item.routeId.routeStop.filter(
                            (from) =>
                                from.stops === req.body.from &&
                                item.Truckdate.setHours(
                                    item.Truckdate.getHours() + from.avgTime
                                ) > new Date()
                        );
                        if (from.length !== 0) return item;
                    } else {
                        const count = item.routeId.routeStop.length;
                        const index = item.routeId.routeStop
                            .map((item) => item.stops)
                            .indexOf(req.body.from);
                        if (index >= 0) {
                            for (let i = 0; i < count; i++) {
                                if (i > index) {
                                    if (
                                        item.routeId.routeStop[i].stops === req.body.destination
                                    ) {
                                        return item;
                                    }
                                }
