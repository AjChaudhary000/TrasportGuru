...
router.post("/verifyuser", async (req, res) => {
    try {
        ...
        if (UserList) {
            ...
            return res
                .status(201)
                .send({ data: UserList, token: token, account: "1", status: true });
        } else {
            const user = new User({
                email: req.body.email,
                accountType: "User",
            ...
            return res
                .status(201)
                .send({ data: data, token: token, account: "0", status: true });
        }
    } catch (e) {
...
router.post("/verifySmsUser", async (req, res) => {
    try {
        ...
        if (UserData === null)
            return res.status(404).send({ data: "otp invalid " });
...
router.post("/googleSignIn", async (req, res) => {
    try {
        ...
        if (UserList) {
            ...
            return res
                .status(201)
                .send({ data: UserList, token: token, status: true });
        } else {
            const user = new User({
                ...req.body,
                accountType: "User",
